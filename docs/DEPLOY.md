...
5. **등록이 끝나면 환경변수를 지워도 된다.** 권한은 이미 DB의 계정 행에 들어 있다.
   ...
   ```
   - ⚠️ **지운 상태에서는 권한 회수도 할 수 없다**(회수도 코드로 확인한다). 아래
     "담당자 교체·코드 유출" 절차는 **코드를 새로 넣는 것부터** 시작한다.
   - 나중에 담당자를 바꾸거나 **DB를 초기화해 재등록해야 할 때** 다시 넣으면 된다.
     그때는 **새 코드를 만들어** 넣는다(이전 코드를 재사용하지 않는다).

### 담당자 교체·퇴사·코드 유출 — 회수 → 재등록 순서로 한다

> **주의**: 예전 문서에는 "담당자를 바꾸려면 코드를 다시 넣으면 된다"고 적혀 있었으나
> **사실이 아니었다.** 코드를 다시 넣고 새 사람이 등록하면 **새 계정에 플래그가 하나 더
> 붙을 뿐, 이전 담당자의 권한은 그대로 남는다.** 반드시 아래 순서로 회수부터 한다.

> **먼저 확인 — 회수 버튼이 화면에 안 보이는 경우(떠난 담당자만 권한이 남은 상황).**
> 🔓 백업 권한 회수 UI는 **💾 데이터 백업 카드 안에** 있고, 그 카드는 **권한이 등록된
> 계정에만** 그려진다. 그래서 **이전 담당자만 권한을 들고 있고 새 담당자는 아직 미등록**
> 이면, 새 담당자 화면에는 회수 버튼 자체가 없어 회수에 도달할 수 없다. 이때는 순서를
> 이렇게 한다: **① 새 담당자가 새 코드로 [권한 등록]을 먼저 해서**(아래 4단계) 백업
> 카드를 띄운다 → **② 그 카드의 🔓 권한 회수로 본인을 포함한 전원을 회수한다**(3단계) →
> **③ 새 담당자가 다시 [권한 등록]한다**(4단계). 회수는 전원을 끄므로 방금 등록한 본인
> 권한도 함께 꺼진다 — 그래서 **마지막에 한 번 더 등록**해야 백업 카드가 남는다.

1. **새 권한 코드를 만들어 환경변수에 넣고 재배포한다**(설정 순서 1~2와 같은 방법).
   이전 코드가 샜다면 이 단계에서 이미 무력화된다 — 등록·회수 모두 **현재 설정된 코드**로만
   된다. 코드를 지워 뒀다면 이 단계가 필수다.
2. **회수할 수 있는 계정으로 로그인한다.** 회수도 **병동 마스터 계정 + 권한 코드**가 필요하다
   (등록과 같은 문을 쓴다 — 회수 쪽만 무르면 그쪽이 구멍이 된다).
3. **⚙️ 설정 → 💾 데이터 백업 카드 → 🔓 백업 권한 회수** 를 펼치고 **새 코드를 입력한 뒤
   [권한 회수]** 를 누른다(확인 창이 한 번 더 뜬다).
   - 회수는 계정을 고르지 않고 **권한을 가진 모든 계정의 플래그를 0으로 되돌린다.** 코드가
     샌 상황에서 정작 문제는 "누가 몰래 등록해 뒀는지 모른다"는 것이라, 한 번에 전부
     정리하는 것이 이 기능의 목적이다.
   - 화면에 **회수된 건수**가 표시된다. **1보다 크면 본인 외의 계정도 권한을 들고 있었다는
     뜻**이므로 그대로 넘기지 말고 `backup_log`의 `granted` 행을 확인한다.
   - 데이터는 지워지지 않는다. 지워지는 것은 권한 플래그뿐이다.
4. **새 담당자가 로그인해 🔐 백업 권한 등록 카드에 새 코드를 입력한다**(설정 순서 4와 동일).
5. 필요하면 5단계처럼 환경변수를 다시 지운다.

API로 확인할 때: `POST /api/admin/backup/revoke {"code": "<현재 코드>"}` →
`{"revoked": <꺼진 계정 수>}`. 코드가 틀리면 403 + `denied` 기록, 잠금 중이면 429다.
회수된 계정마다 `backup_log`에 `revoked` 행이 한 줄씩 남는다(행 수 = 회수 건수).
...
| **같은 계정**에서 5회 실패 | 그 계정만 **15분간 429** — 정답을 넣어도 잠금이 우선한다 |
| 15분 안에 **전체 실패 합계 30회** | 등록·회수 경로가 **전원 15분간 429**(안내 문구가 다르다) |

> **잠금은 계정 축에만 걸고 DB(`users.claim_fails`·`claim_locked_until`)에 저장한다.**
> - 예전에는 출처 IP도 잠금 키에 넣었는데, Railway처럼 **모든 요청이 같은 프록시를 거치는
>   환경에서는 전원이 같은 IP로 보여** 아무나 5회 틀리면 정상 담당자도 15분간 등록하지
>   못하는 사실상의 전역 잠금이었다. IP를 DB에 남기면 백업 ZIP으로 실려 나가기까지 한다
>   (개인정보 — 교훈 L-1). 그래서 **IP는 쓰지 않는다.**
> - 예전에는 카운터가 프로세스 메모리에 있어 **scale-to-zero로 인스턴스가 내려갔다 오면
>   잠금이 풀렸다.** 지금은 계정 행에 있으므로 재시작해도 남은 시간만큼 유지된다.
> - 계정 축만 두면 공격자가 병동을 새로 열어 계정을 만들며 5회씩 무한히 이어갈 수 있다.
>   그래서 **잠금 창이 살아 있는 계정들의 실패 합계**에 상한(30)을 둔다. 자동 대입이
>   15분당 30회 = 하루 2,880회로 묶인다. 대신 **공격이 진행되는 동안에는 정상 담당자의
>   등록도 최대 15분 막힌다** — 등록은 배포 직후·초기화 후에만 하는 1회성 작업이고 15분
>   뒤 저절로 풀리므로, 무제한 대입을 열어 두는 쪽보다 낫다고 보고 택한 절충이다.
> - 시도 사실 자체는 `backup_log`의 `denied` 행으로 영구히 남는다(코드를 아직 설정하지
>   않은 기간의 시도까지 남는다 — 배포 직후가 가장 취약한 시기이기 때문이다).
...
| `denied` | 권한 없는 계정이 반출·권한 등록·회수를 시도했을 때(**권한 코드를 아직 설정하지 않은 기간의 시도 포함**). **uid당 하루 1행으로 합쳐서** 남긴다(로그인만 하면 누구나 두드릴 수 있어 합치지 않으면 진짜 시도가 잡음에 묻힌다) |
| `granted` | 권한 코드로 **등록에 성공**했을 때. 합치지 않고 매번 남긴다 — 코드가 새어 제3의 master가 몰래 등록해도 "언제 누가 권한을 얻었는지"를 사후에 확인할 수 있어야 한다 |
| `revoked` | 권한이 **회수**됐을 때, **회수된 계정마다 한 줄**(actor는 권한을 잃은 계정). 행 수가 곧 회수 건수다 |
...
8. 등록이 끝났으면 **Variables 탭에서 `DUTY_BACKUP_CLAIM_CODE`를 지운다**(§7.6 5단계).
   ...
   - 나중에 **담당자를 바꾸거나 코드가 샜을 때는 새 코드를 다시 넣고 회수 → 재등록**
     순서로 한다(§7.6 "담당자 교체·퇴사·코드 유출"). 코드를 지운 상태에서는 회수도 못 한다.
